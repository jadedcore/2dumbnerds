...
<div class="row">
	<div class="col-md-offset-2 col-md-8">
		...
		<?= $this->Form->create($theUser);?>
			<legend><?= __('Change Password Form');?></legend>
			<div class="row">
				<div class="col-md-12">
					<?php
						$icon = '';
						$class = 'form-group';
						if ($this->Form->isFieldError('Users.currentPassword')) {
							$icon = '<span class="glyphicon glyphicon-remove form-control-feedback" aria-hidden="true"></span>';
							$class .= ' has-feedback has-error';
						}
					?>
					<div class="<?= $class;?>">
						<label class="control-label" for="users-currentpassword">Current Password</label>
						<?=
							$this->Form->control('Users.currentPassword', [
								'label' => false,
								'class' => 'form-control',
								'type' => 'password',
